...
export default {
  ...
  computed: {
    ...
    pageContentDecoded() {
      ...
      try {
        const binary = atob(this.pageContent)
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      ...
      // -> Re-execute scripts inserted via v-html (innerHTML strips script execution)
      // Sequential execution: <script src> loads complete BEFORE inline scripts run,
      // preserving document order semantics (dependencies load first).
      const pageScripts = Array.from(this.$refs.container.querySelectorAll('script'))
      if (pageScripts.length > 0) {
        const execRemaining = (idx) => {
          if (idx >= pageScripts.length) {
            window.boot.notify('page-ready')
            return
          }
          const old = pageScripts[idx]
          const script = document.createElement('script')
          Array.from(old.attributes).forEach(attr => script.setAttribute(attr.name, attr.value))
          old.parentNode.replaceChild(script, old)
          if (old.src) {
            // External script: wait for load (or failure) before continuing
            script.onload = () => execRemaining(idx + 1)
            script.onerror = () => execRemaining(idx + 1)
            script.src = old.src
          } else {
            // Inline script: run synchronously, then continue
